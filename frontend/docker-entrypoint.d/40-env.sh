...
GA_MEASUREMENT_ID_ESCAPED="$(escape_js "${VITE_GA_MEASUREMENT_ID:-}")"
TURNSTILE_SITE_KEY_ESCAPED="$(escape_js "${VITE_TURNSTILE_SITE_KEY:-}")"
...
sed \
  ...
  -e "s|__VITE_GA_MEASUREMENT_ID__|${GA_MEASUREMENT_ID_ESCAPED}|g" \
  -e "s|__VITE_TURNSTILE_SITE_KEY__|${TURNSTILE_SITE_KEY_ESCAPED}|g" \
